## CMA with A/B forced-choice support (llama spm)
#####################################
import os 
import pyrootutils
...
class CustomHookedTransformer(transformer_lens.HookedTransformer):
    def resize_token_embeddings(self, new_num_tokens):
        ...
        HookedTransformer doesn't expose the underlying HF model directly,
        so we manually resize the embedding matrices.
        """
        current_num_tokens = self.W_E.shape[0]
        
        if current_num_tokens == new_num_tokens:
            logger.info(f"Token embeddings already correct size: {current_num_tokens}")
            return
            
        logger.info(f"Resizing embeddings from {current_num_tokens} to {new_num_tokens}")
        
        # Resize input embeddings (W_E)
        old_embeddings = self.W_E.data
        new_embeddings = torch.zeros(new_num_tokens, old_embeddings.shape[1], 
                                    dtype=old_embeddings.dtype, device=old_embeddings.device)
        new_embeddings[:current_num_tokens] = old_embeddings
        
        # Update parameter data in-place
        self.W_E.data = new_embeddings
        
        # Resize output embeddings (W_U) if it exists and has the same vocab size
        if hasattr(self, 'W_U') and self.W_U.shape[-1] == current_num_tokens:
            old_unembeddings = self.W_U.data
            new_unembeddings = torch.zeros(old_unembeddings.shape[0], new_num_tokens,
                                         dtype=old_unembeddings.dtype, device=old_unembeddings.device)  
            new_unembeddings[:, :current_num_tokens] = old_unembeddings
            self.W_U.data = new_unembeddings
            
        # Resize unembed bias (b_U) if it exists and has the same vocab size
        if hasattr(self, 'b_U') and self.b_U.shape[0] == current_num_tokens:
            old_bias = self.b_U.data
            new_bias = torch.zeros(new_num_tokens, dtype=old_bias.dtype, device=old_bias.device)
            new_bias[:current_num_tokens] = old_bias
            self.b_U.data = new_bias
...
def get_args():
    ...
    #### theory of mind task
    parser.add_argument("--use_tom_prompts", action="store_true", help="use theory of mind prompts instead of identity rules")
    parser.add_argument("--tom_locations_file", type=str, default="tom_datasets/locations.txt", help="file containing location phrases for tom prompts")
    parser.add_argument("--tom_format", type=str, default="direct", choices=["direct", "multiple_choice"], help="format for tom prompts: direct (no A/B) or multiple_choice (with A/B)")
...
# during the call to in main, do:
# tokenizer.add_special_tokens({
#     'additional_special_tokens': [LOC_L, LOC_R]
# })
# model.resize_token_embeddings(len(tokenizer))


def generate_tom_prompts(args, tokenizer, locations_file, context_type, rule="ABA", do_shuffle=False, token_sets=None):
    """
    Generate theory of mind prompt pairs for causal mediation analysis.
    
    Args:
        context_type: "abstract" (belief tracking) or "token" (location retrieval)
        rule: "ABA" (false belief) or "ABB" (true belief) for base context
    
    Returns:
        prompt_pairs: list of (base_prompt, exp_prompt) tuples
        correct_ans_pairs: list of (base_ans, exp_ans) tuples  
        causal_ans_list: list of expected answers after patching
    """
    prompt_pairs = []
    correct_ans_pairs = []
    causal_ans_list = []
    ...
    if do_shuffle:
        np.random.shuffle(location_phrases)
        
    while len(prompt_pairs) < args.prompt_num:
        # get locations for this scenario
        if token_sets is None:
            locations = random.sample(location_phrases, k=(args.in_context_example_num + 1) * 2)
        else:
            locations = token_sets.pop(0)
            
        # build few-shot examples
        base_examples = []
        exp_examples = []
        
        for idx in range(args.in_context_example_num):
            loc_a = locations[idx*2]
            loc_b = locations[idx*2 + 1]
            
            if rule == "ABA":  # false belief base pattern
                if args.tom_format == "multiple_choice":
                    base_scenario = f"object is located at {LOC_L}{loc_a}{LOC_R}. agent leaves room. object moves to {LOC_L}{loc_b}{LOC_R}. agent returns and looks where?\nA: {LOC_L}{loc_a}{LOC_R}\nB: {LOC_L}{loc_b}{LOC_R}\nanswer: A"
                else:  # direct format
                    base_scenario = f"object is located at {LOC_L}{loc_a}{LOC_R}. agent leaves room. object moves to {LOC_L}{loc_b}{LOC_R}. agent returns and looks where? {LOC_L}{loc_a}{LOC_R}"

                if context_type == "abstract":
                    # swap belief states - exp becomes true belief 
                    if args.tom_format == "multiple_choice":
                        exp_scenario = f"object is located at {LOC_L}{loc_b}{LOC_R}. object moves to {LOC_L}{loc_a}{LOC_R}. agent leaves room. agent returns and looks where?\nA: {LOC_L}{loc_b}{LOC_R}\nB: {LOC_L}{loc_a}{LOC_R}\nanswer: B"
                    else:  # direct format
                        exp_scenario = f"object is located at {LOC_L}{loc_b}{LOC_R}. object moves to {LOC_L}{loc_a}{LOC_R}. agent leaves room. agent returns and looks where? {LOC_L}{loc_a}{LOC_R}"
                else:
                    # same scenario, test literal location retrieval
                    exp_scenario = base_scenario
                    
            elif rule == "ABB":  # true belief base pattern
                if args.tom_format == "multiple_choice":
                    base_scenario = f"object is located at {LOC_L}{loc_a}{LOC_R}. object moves to {LOC_L}{loc_b}{LOC_R}. agent leaves room. agent returns and looks where?\nA: {LOC_L}{loc_a}{LOC_R}\nB: {LOC_L}{loc_b}{LOC_R}\nanswer: B"
                else:  # direct format
                    base_scenario = f"object is located at {LOC_L}{loc_a}{LOC_R}. object moves to {LOC_L}{loc_b}{LOC_R}. agent leaves room. agent returns and looks where? {LOC_L}{loc_b}{LOC_R}"

                if context_type == "abstract":
                    # swap to false belief
                    if args.tom_format == "multiple_choice":
                        exp_scenario = f"object is located at {LOC_L}{loc_b}{LOC_R}. agent leaves room. object moves to {LOC_L}{loc_a}{LOC_R}. agent returns and looks where?\nA: {LOC_L}{loc_b}{LOC_R}\nB: {LOC_L}{loc_a}{LOC_R}\nanswer: A"
                    else:  # direct format
                        exp_scenario = f"object is located at {LOC_L}{loc_b}{LOC_R}. agent leaves room. object moves to {LOC_L}{loc_a}{LOC_R}. agent returns and looks where? {LOC_L}{loc_b}{LOC_R}"
                else:
                    exp_scenario = base_scenario
                    
            base_examples.append(base_scenario)
            exp_examples.append(exp_scenario)
        
        # generate test question (incomplete)
        test_loc_a = locations[-2] 
        test_loc_b = locations[-1]
        
        if rule == "ABA":  # false belief base pattern
            base_question = f"object is located at {LOC_L}{test_loc_a}{LOC_R}. agent leaves room. object moves to {LOC_L}{test_loc_b}{LOC_R}. agent returns and looks where?"
            base_ans = test_loc_a  # agent has false belief
            
            if context_type == "abstract":
                # exp context: true belief scenario (agent witnesses move)
                exp_question = f"object is located at {LOC_L}{test_loc_a}{LOC_R}. object moves to {LOC_L}{test_loc_b}{LOC_R}. agent leaves room. agent returns and looks where?"
                exp_ans = test_loc_b  # agent has true belief, looks at actual location
                causal_ans = test_loc_a  # after patching belief heads, true belief should become false belief
            else:
                # exp context: same scenario, different phrasing
                exp_question = f"object is located at {LOC_L}{test_loc_a}{LOC_R}. agent leaves room. object moves to {LOC_L}{test_loc_b}{LOC_R}. where does agent look?"
                exp_ans = test_loc_a  # same false belief answer
                causal_ans = test_loc_a  # literal location retrieval should stay same
                
        elif rule == "ABB":  # true belief base pattern
            base_question = f"object is located at {LOC_L}{test_loc_a}{LOC_R}. object moves to {LOC_L}{test_loc_b}{LOC_R}. agent leaves room. agent returns and looks where?"
            base_ans = test_loc_b  # agent has true belief
            
            if context_type == "abstract":
                # exp context: false belief scenario (agent misses move) 
                exp_question = f"object is located at {LOC_L}{test_loc_a}{LOC_R}. agent leaves room. object moves to {LOC_L}{test_loc_b}{LOC_R}. agent returns and looks where?"
                exp_ans = test_loc_a  # agent has false belief
                causal_ans = test_loc_b  # after patching belief heads, false belief should become true belief
            else:
                # exp context: same scenario, different phrasing
                exp_question = f"object is located at {LOC_L}{test_loc_a}{LOC_R}. object moves to {LOC_L}{test_loc_b}{LOC_R}. where does agent look?"
                exp_ans = test_loc_b  # same true belief answer  
                causal_ans = test_loc_b  # literal location should stay same
        
        # construct full prompts based on format
        base_prompt = "\n".join(base_examples + [base_question])
        exp_prompt = "\n".join(exp_examples + [exp_question])
        
        if args.tom_format == "multiple_choice":
            # add multiple choice format with A/B options
            base_prompt += f"\nA: {LOC_L}{test_loc_a}{LOC_R}\nB: {LOC_L}{test_loc_b}{LOC_R}\nanswer:"
            exp_prompt += f"\nA: {LOC_L}{test_loc_a}{LOC_R}\nB: {LOC_L}{test_loc_b}{LOC_R}\nanswer:"
            # convert answers to A/B format
            base_ans_formatted = "A" if base_ans == test_loc_a else "B"
            exp_ans_formatted = "A" if exp_ans == test_loc_a else "B" 
            causal_ans_formatted = "A" if causal_ans == test_loc_a else "B"
        else:  # direct format
            # direct answer format - model should generate location directly
            base_prompt += f" {LOC_L}"
            exp_prompt += f" {LOC_L}"
            # keep original location answers
            base_ans_formatted = base_ans
            exp_ans_formatted = exp_ans
            causal_ans_formatted = causal_ans
        
        # check for duplicates
        if (base_prompt, exp_prompt) not in prompt_pairs:
            prompt_pairs.append((base_prompt, exp_prompt))
            correct_ans_pairs.append((base_ans_formatted, exp_ans_formatted))
            causal_ans_list.append(causal_ans_formatted)
        else:
            if args.verbose:
                logger.info(f"Repeated prompt pair: {base_prompt[:50]}...")
                
    return prompt_pairs, correct_ans_pairs, causal_ans_list
...
def cal_ab_margin(logits, A_tok_id: int, B_tok_id: int):
    """
        compute the next-token margin for forced-choice A vs B at the final position:
        margin = logit('A') - logit('B').
        positive => prefer A, negative => prefer B.
    """
    if isinstance(A_tok_id, torch.Tensor):
        A_tok_id = A_tok_id.item()
    if isinstance(B_tok_id, torch.Tensor):
        B_tok_id = B_tok_id.item()
    return (logits[0, -1, A_tok_id] - logits[0, -1, B_tok_id])
...
def generate_response_eval(
    ...
    """
    # Filter out forced-choice parameters that don't belong in generation
    gen_kwargs = {k: v for k, v in generation_kwargs.items() 
                  if k not in ['A_tok_id', 'B_tok_id', 'eval_metric']}
    ...
    for response_id in response_ids:
        ...
        # for TOM prompts, truncate at </loc> to get clean location answer
        if hasattr(args, 'use_tom_prompts') and args.use_tom_prompts:
            if '</loc>' in gen_ans:
                gen_ans = gen_ans.split('</loc>')[0]
                logger.debug(f"Truncated TOM response to: '{gen_ans}'")
            else:
                logger.warning(f"TOM response missing </loc> tag: '{gen_ans}'")
...
def ablate_head(
        ...
        ## forced-choice A/B metric support
        eval_metric="gen_acc",
        A_tok_id=None, B_tok_id=None,
        **generation_kwargs
        ):
    """
    ...
    for layer_idx in tqdm(range(total_layers)):
        ...
        for head_idx in range(total_heads):            
            if generate: ## generating the response while implementing activation patching during the run
                ...
                if eval_metric == "ab_forced":
                    patched_score = cal_ab_margin(patched_logits, A_tok_id, B_tok_id)
                    logits_diff_change[layer_idx, head_idx] = patched_score - exp_logits_diff
                else:
                    patched_exp_logits_diff =  cal_logit_prob_diff(
                        patched_logits, model, causal_ans=patched_exp_ans, original_ans=exp_ans)
                    logits_diff_change[layer_idx, head_idx] = patched_exp_logits_diff -  exp_logits_diff ## change in logits difference after patching
    ...
    for layer_idx in tqdm(range(total_layers)):
        for token_pos in range(num_position):
            ...
            if generate:
                ...
                if eval_metric == "ab_forced":
                    patched_score = cal_ab_margin(patched_logits, A_tok_id, B_tok_id)
                    logits_diff_change[layer_idx, token_pos] = patched_score - exp_logits_diff
                else:
                    patched_exp_logits_diff =  cal_logit_prob_diff(
                        patched_logits, model, causal_ans=patched_exp_ans, original_ans=exp_ans)
...
def activation_patching(
    ...
    for idx_, pair in enumerate(prompt_pairs):
        ...
        A_tok_id = generation_kwargs.get("A_tok_id", None)
        B_tok_id = generation_kwargs.get("B_tok_id", None)
        ...
        if eval_metric == "gen_acc":
            ...
            **generation_kwargs)
            if base_acc < low_prob_threshold:
                ...
                continue
        elif eval_metric == "ab_forced":
            base_logits = model(base_input_ids)
            base_margin = cal_ab_margin(base_logits, A_tok_id, B_tok_id)
            base_pred = "A" if base_margin.item() > 0 else "B"
            if base_pred != base_ans:
                logger.warning(f"base context: wrong A/B ({base_pred} != {base_ans})")
                continue
        ...
        if eval_metric == "gen_acc":
            ...
            **generation_kwargs)
            ...
            exp_margin = cal_ab_margin(exp_logits, A_tok_id, B_tok_id)
            exp_pred = "A" if exp_margin.item() > 0 else "B"
            if exp_pred != exp_ans:
                logger.warning(f"exp context: wrong A/B ({exp_pred} != {exp_ans})")
                continue

        exp_logits = model(exp_input_ids)
        if eval_metric == "ab_forced":
            exp_logits_diff = cal_ab_margin(exp_logits, A_tok_id, B_tok_id)
        else:
        ...
        ## pass A/B token ids for forced-choice metric
        kwargs.update({"A_tok_id": A_tok_id, "B_tok_id": B_tok_id, "eval_metric": eval_metric})
...
def main(args):
    ...
    # add special tokens before loading model if using tom prompts
    if args.use_tom_prompts:
        tokenizer.add_special_tokens({
            'additional_special_tokens': [LOC_L, LOC_R]
        })
        logger.info(f"Added tom location tokens: {LOC_L}, {LOC_R}")

    ## Load the model
    try:
        # For models not officially supported by transformer_lens, try loading directly first
        if model_family == "gpt-oss":
            logger.info(f"Attempting to load {model_id} directly (experimental support)")
            # Try loading directly - transformer_lens might auto-detect config
            try:
                ...
                # Let transformer_lens infer config from the model
                model = CustomHookedTransformer.from_pretrained(
                    model_id,  # try direct loading first
                    hf_model=hf_model,
                    tokenizer=tokenizer,
                    device_map=args.device_map, 
                    device=args.device,
                ...
                logger.error(f"gpt-oss models use MoE architecture not fully supported by transformer_lens")
                logger.error(f"Consider using a standard transformer model for CMA analysis")
                raise ValueError(f"gpt-oss-20b architecture (MoE, GQA) not compatible with transformer_lens activation patching")
        else:
            model = CustomHookedTransformer.from_pretrained(
                model_id, 
                device_map=args.device_map, 
    ...
    ## setup single-token A/B for forced-choice (llama spm often needs a leading space)
    tokA = tokenizer.tokenize(" A")
    tokB = tokenizer.tokenize(" B")
    if len(tokA) == 1 and len(tokB) == 1:
        A_tok_id = tokenizer.convert_tokens_to_ids(tokA[0])
        B_tok_id = tokenizer.convert_tokens_to_ids(tokB[0])
        logger.info(f"using single-token answers: {tokA[0]} / {tokB[0]}")
    else:
        tokenizer.add_special_tokens({'additional_special_tokens': ['<A>', '<B>']})
        model.resize_token_embeddings(len(tokenizer))
        A_tok_id = tokenizer.convert_tokens_to_ids('<A>')
        B_tok_id = tokenizer.convert_tokens_to_ids('<B>')
        logger.info("added <A>/<B> as special tokens for forced-choice answers")
    ...
    if args.use_tom_prompts:
        prompts, correct_ans_list, causal_ans_list = generate_tom_prompts(
            args, tokenizer, args.tom_locations_file, context_type=args.context_type,
            rule=args.base_rule, do_shuffle=args.do_shuffle,
        )
    else:
    ...
    activation_patching(
        ...
        A_tok_id=A_tok_id, B_tok_id=B_tok_id,
        **generation_kwargs
    )
